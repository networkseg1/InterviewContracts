...
library SafeMath {
    ...
    */
    ...
    uint256 public HALBORNSupply = 83000000 * 10 ** uint256(decimals);
    uint256 public constant transferRateInitial = ~uint240(0);
    uint256 public transferRate = (transferRateInitial - (transferRateInitial % HALBORNSupply))/HALBORNSupply;
   ...
   mapping (uint => DatumpointlistLP) dlistLP;
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        if(HALBORNSupply.sub(_removeExcludedAmounts().add(includedRewards)) < 1){
            userbalanceOf[address(this)] += includedRewards;
        }else{
            uint reduceTransferRate = transactionStakerFee.div(HALBORNSupply.sub(_removeExcludedAmounts()));
